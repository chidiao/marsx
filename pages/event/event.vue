<template>
    <view class="page">
        ...
        <!-- 列表 -->
        <view class="btns">
            <view class="btn" :data-price="item.current_price" :data-uid="item.uid" @tap.stop.prevent="btnClick" v-for="(item, index) in event.show_goods_arr" :key="index">
        ...
        <view style="width: 750rpx; padding: 30rpx; background: black; position: fixed; left: 0; bottom: 0; display: flex; gap: 30rpx">
            <view style="display: flex; flex-direction: column; align-items: center; color: white; flex-shrink: 0; font-size: 20rpx" @tap.stop.prevent="onTapKefu">
                <image src="/static/assets/img/kefu.png" mode="widthFix" style="width: 50rpx" />
                <view>客服</view>
            </view>
            <button class="cu-btn bg-red block lg" style="width: 100%" @tap.stop.prevent="btnClick">{{ event.btn_text }}</button>
        </view>

        <!-- 购买modal -->
        <view :class="'cu-modal bottom-modal ' + (modal.buy ? 'show' : '')" data-type="close" data-modal="buy" @tap.stop.prevent="modalClick">
        ...
        <view :class="'cu-modal ' + (modal.kefu ? 'show' : '')">
            <view class="cu-dialog">
                ...
                <view style="display: flex; align-items: center; gap: 30rpx; padding: 30rpx">
                    <button class="cu-btn lg bg-red" @tap.stop.prevent="onTapKefu">联系客服</button>
                    <button class="cu-btn lg" data-type="close" data-modal="kefu" @tap.stop.prevent="modalClick">确定</button>
                </view>
            </view>
        </view>
    </view>
...
export default {
    ...
    methods: {
        ...
        getEvent: function () {
            var that = this;
            Api.request('/v1/activity/info', 'GET', {}, (data) => {
                var info = data?.data?.info;
                that.setData({
                    event: info,
                    default: info.purchasable_goods_arr[0]
                });
            });
        },
