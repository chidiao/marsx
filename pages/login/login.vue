<template>
    <view style="height: 100vh; width: 100vw; position: relative" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd">
      ...
      <view class="info"><strong>作品数：</strong>78</view> -->
        ...
        <view :style="'background-color: #222;transform: translateY(' + translateY + 'vh);'" @tap="onTapCloseAll">
            ...
            <view class="content page4" v-if="show_content_page == 3">
                ...
                <view class="desc">私域派对将为您提供线上聚会的场地，邀请朋友一起来MarsX 元宇宙世界举办线上派对，加入律动舞池，互娱私域，让我们一起尽情律动放飞自我吧。</view>
            </view>
            <video
                id="myVideo4"
                class="myVideo"
        ...
        <view class="btns">
            ...
            <view v-if="show_login_btn && platform == 'android' && show_user" class="btn btn2" @tap.stop.prevent="onTapLogin">用户权益</view>
            ...
            <view class="login-box" v-if="show_login_box && !show_user">
                ...
                <view v-if="show_login_btn && !show_user && need_mobile == 0" class="btn btn2" @tap.stop.prevent="onTapLogin">一键登录</view>
                <button v-if="show_login_btn && !show_user && need_mobile == 1" class="btn btn2" open-type="getPhoneNumber" :disabled="!agreement" @getphonenumber="getPhoneNumber">
                    一键登录
                </button>
            ...
            <view class="cards">
                <view class="card" v-for="(item, index) in packageData" :key="index">
                    <view class="info">
                        ...
                        <view class="price">
                            ￥{{ item.current_price }}
                            <span class="original_price" v-if="item.current_price != item.original_price">￥{{ item.original_price }}</span>
                        </view>
                    </view>

                    <view class="btn-buy btn-red" @tap.stop.prevent="goPay" :data-price="item.current_price" :data-uid="item.uid" v-if="item.status == 1">购买</view>
...
export default {
     ...
     */,
    onLoad(options) {
        // getApp().audioManager.playBgSound1();
    },
    /**
     * 生命周期函数--监听页面初次渲染完成
     */
            ...
            Api.request(
                ...
                (data) => {
                    ...
                    if (data?.data?.token) {
                        ...
                        getApp().globalData.setGlobalData('token', data.data.token);
                        this.setData({
                            show_user: true,
                ...
                success: (res) => {
                    ...
                    Api.request(
                        ...
                        (data) => {
                            ...
                            this.reportLog('wx_open_id: ' + data?.data?.user_info?.wx_open_id + ' need_mobile:' + data?.data?.need_mobile);
                            this.setData({
